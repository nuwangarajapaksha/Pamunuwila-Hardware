...
public class LoginFormModel {
    ...
    public boolean checkUsers() {
        String query = "SELECT count(user_no) FROM pamunuwila_hardware_db.user";
        logger.debug("SQL query String: " + query);
        ...
        try {
            ...
            while (rs.next()) {
                int userCount = rs.getInt("count(user_no)");
                if (userCount == 0) {
                    giveAccess = true;
                    logger.debug("give access");
                }
        ...
        logger.debug("return give access");
        return giveAccess;
    }
